...
// MARK: custom routes
public class Controller: StandardController {
    var fullyInitialized = true
    
    override func setupBasicRoutes(router:Router) {
        
        super.setupBasicRoutes(router: router)
...
extension  Controller {
    ...
    func loadReportsInfo(_ s:String)->[String:Any] {
        return ["report-list":s,"data": Controller.reportsDict()]
    }
    ...
    public func getJSON(request: RouterRequest, response: RouterResponse, next: @escaping () -> Void) throws {
        global.apic.getIn += 1
        try finishJSONStatusResponse(["reports-list":Controller.reportsDict()], request: request, response: response, next: next)
